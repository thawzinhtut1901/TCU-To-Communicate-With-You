...
const ChatList = () => {
    ...
    return (
        <div className="flex flex-col max-h-[70vh] overflow-auto scrollbar-hide">
            ...
            {/* {chatData?.map((socketChat: any) => (
                <div
                onClick={() => handleChatting(socketChat?.id)}
                key={socketChat?.id}
                className="flex items-center border-[#591DA9] bg-[#2264E5] bg-opacity-10 mx-auto mb-4 border rounded-[12px] w-11/12 h-[88px] cursor-pointer"
                >
                <img src={socketChat?.profile} alt="" className="mx-[12px] rounded-full w-[64px] h-[64px]" />
                <div className="flex flex-col font-primary">
                    <h1 className="font-medium text-[20px]">{socketChat?.displayName}</h1>
                </div>
                </div>
            ))} */}
        </div>
